...
function testLetterToColOffset () {
  for (let col of ['A','B','Z','AA','AZ','BA','ZZ','AAA','AAB','ABA','QR','&@']) {
    try {
...
function a1rangeMatch (target, cell) {
  ...
  if (target.includes(':')) {
    ...
    if (startCol===null || startCol <= cellCol) { // Match start col
      if (endCol===null || endCol >= cellCol) { // match end col
        if (startRow===null || startRow <= cellRow) { // match start row
          if (endRow===null || endRow >= cellRow) { // match end Row
            return true;
          }
        } 
      }
...
addTriggerCell({ cell, sheet, rangeName, callback, setup = () => {} }) {
  ...
  if (rangeName) {
    ...
    if (namedRange) {
      ...
      if (this.debug) {
        console.log(`Using existing named range: ${rangeName} (${cell} in sheet ${sheet})`);
      }
    } else {
      if (!cell) {
        throw new Error(`Named range ${rangeName} does not exist, and cell not provided to create it.`);
      }
      const sheetObj = sheet ? ss.getSheetByName(sheet) : ss.getActiveSheet();
      namedRange = sheetObj.getRange(cell);
      ss.setNamedRange(rangeName, namedRange);
      sheet = sheetObj.getName();
      if (this.debug) {
        console.log(`Created new named range: ${rangeName} (${cell} in sheet ${sheet})`);
      }
    }
  } else {
    if (!sheet) {
...
addDropdown({ cell, sheet, rangeName, values, callbacks, setup=()=>{}}) {
  const setupValidation = () => {
    const range = rangeName
      ...
      : (sheet
        ? SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheet).getRange(cell)
  ...
  this.addTriggerCell({ cell, sheet, rangeName, callback, setup : ()=>{
    setup();
    setupValidation();
  } });
}
...
addCheckbox({ cell, sheet, rangeName, onTrue, onFalse, setup=()=>{} }) {
  ...
  this.addTriggerCell({ cell, sheet, rangeName, callback, setup : ()=>{setup();setupValidation();} });
}
  ...
  onEdit(e) { 
    ...
    if (this.debug) {
      console.log('onEdit got',editedCell,editedCells.map((c)=>c.a1),e.oldValue);
    }
    let triggered = false;
    this.triggers.forEach(trigger => {
      if (!trigger.sheet || trigger.sheet == editedSheet.getName()) {
        let triggeredCells = editedCells.filter((cell)=>a1rangeMatch(trigger.cell,cell.a1));
        if (triggeredCells.length) {
          ...
          for (let cell of triggeredCells) {
            if (this.debug) console.log('Callback for ',cell.a1,'from',trigger.cell)
            trigger.callback(
            {
              value : cell.value,
              range: cell.range,
    ...
    if (this.debug && !triggered) {
      console.log('No rule matched from',this.triggers.map((t)=>t.cell))
    }
  }
  ...
  isCellInRange(editedCell, triggerRange) {
    ...
    return range.getA1Notation() === editedCell || range.getA1Notation().includes(editedCell);
  }
}
...
function initializeTriggers() {
  ...
  for (const trigger of triggers) {
    if (trigger.getHandlerFunction() === 'onEvent' && trigger.getEventType() === ScriptApp.EventType.ON_EDIT) {
