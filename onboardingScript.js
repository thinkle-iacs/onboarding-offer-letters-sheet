...
const headers = [LASTFIRST, FIRST,LAST,PERSONAL_EMAIL, IACS_ACCOUNT,ACTION,LOG,OU,POSITION,SCHOOL,DEPT];
...
function getOnboardCols () {
  let onboardSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(ONBOARD_SHEET);
...
addDropdown(
  {cell : actionCells,
  sheet: ONBOARD_SHEET,
  rangeName: 'action_range',
  values : ['-','Create Email','Add to Google Apps Tool','Add to Onboarding'],
  callbacks : {
    'Create Email' : ({values,a1})=>{
      ...
      onboardSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(ONBOARD_SHEET);
      let firstName = onboardSheet.getRange(`${headerMap[FIRST]}${row1based}`).getValue();
      let lastName = onboardSheet.getRange(`${headerMap[LAST]}${row1based}`).getValue();
      let ou = onboardSheet.getRange(`${headerMap[OU]}${row1based}`).getValue();
      let position = onboardSheet.getRange(`${headerMap[POSITION]}${row1based}`).getValue();
      let personalEmail = onboardSheet.getRange(`${headerMap[PERSONAL_EMAIL]}${row1based}`).getValue();
      let school = onboardSheet.getRange(`${headerMap[SCHOOL]}${row1based}`).getValue();
      ...
      // Generate email...
      let iacsEmailRange = onboardSheet.getRange(`${headerMap[IACS_ACCOUNT]}${row1based}`);
      let iacsEmail = iacsEmailRange.getValue();
      let generatedEmail = '';
      if (!iacsEmail) {
        generatedEmail = sanitizeName(firstName[0] + lastName) + '@innovationcharter.org';        
...
Recovery email: ${userAsCreated.recoveryEmail}
        ...
        SpreadsheetApp.getActiveSpreadsheet().toast(
          `A user with the email address ${user.primaryEmail} already exists. Try changing the name in the ${IACS_ACCOUNT} field
          if this account should be given a different address. If this user is the *same* user, you can skip the Create Email step.`
        );
        } else {
...
`;
      GmailApp.sendEmail(personalEmail, mailSubject, mailBody);  
       SpreadsheetApp.getActiveSpreadsheet().toast('Created email and sent credentials to '+personalEmail);
      }
    },   
    'Add to Onboarding' : ({values,a1,range}) => {
      SpreadsheetApp.getActiveSpreadsheet().toast('Copying row into Onboarding sheet');
      let rowNum = range.getRow();
      const target = "https://docs.google.com/spreadsheets/d/1VQ9-z3uTLnTIEHtMbyFiEbWL2G-TgVR0RQhXSFzbBGo/edit#gid=1225031524"
      let cols = getOnboardCols();
      let sheet = range.getSheet();
      let namedValues = {};
      console.log('Acting on',rowNum,values,a1,range.getA1Notation(),sheet.getName())
      for (let col in cols) {
        let a1 = cols[col] + rowNum;
        namedValues[col] = sheet.getRange(a1).getValue();
      }
      // inserting magic value...
      namedValues['Google Onboarding Link'] = `https://tinyurl.com/iacs-onboard?u=${namedValues[IACS_ACCOUNT].split('@')[0]}`;
      console.log('Got vals',namedValues);
      var targetSheet;
      try {
        targetSheet = SpreadsheetApp.openByUrl(target).getSheetByName("New Hires");
      } catch (err) {
        SpreadsheetApp.alert(`Unable to find sheet named ${targetSheet} in onboarding sheet at ${target}.`)
        return;
      }
      let headerRow = 2;
      let headerNames = targetSheet.getRange(`${headerRow}:${headerRow}`).getValues()[0];
      let newRow = [];
      let missingCols = [];
      ...
      if (newRow.length) {
        ...
        SpreadsheetApp.getActiveSpreadsheet().toast('Done copying row into Onboarding sheet!')
      } else {
        SpreadsheetApp.getUi().alert(`Unable to copy data. No matching column headers found; we expected them in row ${headerRow} of the sheet named ${sheet} in the onboarding spreadsheet.`);
      }
    }
  }}
)
